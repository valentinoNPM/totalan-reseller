-- Apply four user-approved canonical name corrections atomically.
DO $$
DECLARE
  v_rename record;
  v_product_id uuid;
BEGIN
  FOR v_rename IN
    SELECT * FROM (VALUES
      ('DANIA PJPD STD'::text, 'DANIA PJPD STANDAR'::text),
      ('OLIVE STD'::text,      'OLIVE STANDAR'::text),
      ('YOONA PJPD STD'::text, 'YOONA PJPD STANDAR'::text),
      ('PDPD STD'::text,       'PDPD STANDAR'::text)
